...
func TestToAnthropicTools(t *testing.T) {
	tools, err := toAnthropicTools(context.Background(), []ToolSpec{{
		Name:        "weather",
		Description: "查询城市天气",
...
func TestToAnthropicMessagesRequiresToolUseID(t *testing.T) {
	_, err := toAnthropicMessages(context.Background(), []Message{{Role: RoleTool, ToolName: "weather", Content: "北京：晴，25°C"}})
	if err == nil {
...
func TestToAnthropicMessagesWithToolResult(t *testing.T) {
	_, err := toAnthropicMessages(context.Background(), []Message{
